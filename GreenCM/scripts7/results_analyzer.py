...
for f in files:
	...
	for line in f:
		...
		if "throughput" in line:
			commits = float(line.split(":")[1].strip().split(" ")[0])
		if "total aborts" in line:
			aborts = float(line.split(":")[1].strip().split(" ")[0])
	f.close()
	affectedpower = 0
	if time == -1 or commits == -1 or aborts == -1 or len(power) == 0:
		continue
	else:
		...
		runs[threads]["edp"].append(100000.00/commits*affectedpower)
		runs[threads]["edp_t"].append(100000.00/commits*runs[threads]["power_t"][-1])

#runs = [(k,v) for (k,v) in runs.items()]
#runs.sort()
print "threads\tcommits\taborts\tratio\tretries\tpower\tpower_t\ttime\tEDP\tEDP_t\trtime\tpower_std\tpowert_std\ttime_std\tedp_std\tedpt_std"
for k in avail_threads:
	...
	out += "\t"+str(powert_avg)
        ...
        edp_t_std =  numpy.std(v["edp_t"])
        if edp_t_std/edpt_avg * 100 > 10:
                sys.stderr.write(str(k) + ", edp_t, " + str(edp_t_std/edpt_avg) + "\n")

	if len(v["time"]) > 5:
		rtime_avg = numpy.average(v["time"])
	else:
		rtime_avg = numpy.median(v["time"])
	time_avg = 100000.00/commits_avg
	commits_std =  numpy.std(v["commits"])
	if commits_std/commits_avg * 100 > 10:
		sys.stderr.write(str(k) + ", commits, " + str(commits_std/commits_avg) + "\n")
	out += "\t"+str(time_avg)
	out += "\t"+str(edp_avg)
	out += "\t"+str(edpt_avg)
        out += "\t"+str(rtime_avg)
