...
for x in 500 1500 5000 10000 20000 50000 80000 100000 120000 150000
do
export SPINTOSLEEP=$x
for f in 0 2 4 8 16
do
for b in  2 3 4
do
		...
		export FERRARIS=$f
                cd $workspace;
                cd ${executables[$b]};
                echo "${backend[$c]} | ${benchmarks[$b]} | threads $t | threshold $x | ferraris $FERRARIS | attempt $a"
                ./${executables[$b]} ${params[$b]}$t > $runsdir/${benchmarks[$b]}-${backend[$c]}-$x-$f-$t-$a.data 2> $runsdir/${benchmarks[$b]}-${backend[$c]}-$x-$f-$t-$a.err &
		pid3=$!
...
do
    ...
    do
	    ...
	    cd $workspace/mcd;
            ...
            for a in {1..5}
            do
                echo "${bStr[$b]} | backend ${backend[$be]} | threshold $x | ferraris $FERRARIS  | threads $t | attempt $a"
		#let ferraris=$t/4
                #if [ $ferraris -le 1 ]; then
                #       let ferraris=2;
                #fi 
                export FERRARIS=$f
                ./memcached -p 20000 -t $t > $runsdir/${bStr[$b]}-${backend[$be]}-$x-$f-$t-$a.eng 2> $runsdir/${bStr[$b]}-${backend[$be]}-$x-$f-$t-$a.freq & 
                pid_server=$!
		echo "memcslap -F $workspace/scriptsm/memslap-${benchmarks[$b]}.cnf -s 127.0.0.1:20000 -c $t -T $t -x 625000 -B > $runsdir/${bStr[$b]}-${backend[$be]}-$x-$f-$t-$a.data 2> $runsdir/${bStr[$b]}-${backend[$be]}-$x-$f-$t-$a.err"
                memcslap -F $workspace/scriptsm/memslap-${benchmarks[$b]}.cnf -s 127.0.0.1:20000 -c $t -T $t -x 625000 -B > $runsdir/${bStr[$b]}-${backend[$be]}-$x-$f-$t-$a.data 2> $runsdir/${bStr[$b]}-${backend[$be]}-$x-$f-$t-$a.err &
                pid_client=$!
                wait_until_finish $pid_client; wait $pid_client; rc=$?
                if [[ $rc != 0 ]] ; then
                    echo "Error within: backend ${backend[$be]} | ${bStr[$b]} | htm_retries $retries | apriori tries $apriori | threads $t | attempt $a" >> $runsdir/error.out
                fi
...
do
    ...
    do
        ...
        #do
            ...
            for a in {1..5}
            do
                echo "${bstr[$b]} | ${backend[$c]} | threads $t | threshold $x | ferraris $ferraris | attempt $a"
                ./stmbench7-orig/sb7_tt -i false -s b -h false -r false ${benchmarks7[$b]} -d 30000 -n $t > $runsdir/${bstr[$b]}-${backend[$c]}-$x-$f-$t-$a.data  2> $runsdir/${bstr[$b]}-${backend[$c]}-$x-$f-$t-$a.out
                rc=$?
                if [[ $rc != 0 ]] ; then
